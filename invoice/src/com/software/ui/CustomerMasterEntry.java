...
public class CustomerMasterEntry extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(invoice.InvoiceApp.class).getContext().getResourceMap(CustomerMasterEntry.class);
...
public void populateCustomerCode(){
    ...
    int counter=0;
    String query="select count(*) from client_master ";
    try{
...
public boolean verifyFields(){
        ...
        if(consumerCode.trim().length()>0 && consumerName.trim().length()>0 && address1.trim().length()>0 && address2.trim().length()>0 && pin.trim().length()>0 && state.trim().length()>0 )
            return true;
        else return false;
